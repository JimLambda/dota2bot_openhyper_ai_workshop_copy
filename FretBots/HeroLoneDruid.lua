...
local d = 5
local e = 120
local f = "bearSpawnTimerName"
local g = 10
local h = {}
local i = "itemCheckTimerName"
function HeroLoneDruid:ItemCheckTimer()
	for j = 1, 16 do
		local k = unit:GetItemInSlot(j)
		if k ~= nil then
			replacedItem = k:GetName()
		end
	end
end
function HeroLoneDruid:BearSpawnTimer()
	local l = false
	local m = HeroLoneDruid:FindBear()
	if m ~= nil and m:IsAlive() then
		Debug:Print("HeroLoneDruid: Bear Found. Starting LoneDruid item event watcher.")
		HeroLoneDruid.Bear = m
		Timers:CreateTimer(i, { endTime = 1, callback = HeroLoneDruid["ItemCheckTimer"] })
...
function HeroLoneDruid:Initialize()
	...
	for n, unit in pairs(AllUnits) do
	...
	if HeroLoneDruid.Hero == nil then
		Debug:Print(
			"Lone Druid Specific Extensions are enabled, but Lone Druid is not present.  HeroLoneDruid Exiting."
		)
		return
	end
	Timers:CreateTimer(f, { endTime = 1, callback = HeroLoneDruid["BearSpawnTimer"] })
end
