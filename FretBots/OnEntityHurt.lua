...
function EntityHurt:OnEntityHurt(c)
	isHero = EntityHurt:GetIsHero(c)
	if not isHero then
		return
	end
	victim, attacker, damage, damageType = EntityHurt:GetEntityHurtEventData(c)
	if victim == nil then
		return
	end
	if victim.stats == nil then
		return
	end
...
function EntityHurt:GetIsHero(c)
	local isHero = false
	local victim = EntIndexToHScript(c.entindex_killed)
	if victim:IsHero() and victim:IsRealHero() and not victim:IsIllusion() and not victim:IsClone() then
		isHero = true
	end
	return isHero
end
function EntityHurt:GetEntityHurtEventData(c)
	local attacker = nil
	local victim = nil
	if c.entindex_attacker ~= nil and c.entindex_killed ~= nil then
		attacker = EntIndexToHScript(c.entindex_attacker)
		victim = EntIndexToHScript(c.entindex_killed)
	end
	local damageType = nil
	if c.entindex_inflictor ~= nil then
		inflictor_table = EntIndexToHScript(c.entindex_inflictor):GetAbilityKeyValues()
		if inflictor_table["AbilityUnitDamageType"] == nil then
