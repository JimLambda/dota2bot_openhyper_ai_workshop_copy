...
local e = false
local f = 16
local g, h = pcall(require, "bots.FretBots.SettingsNeutralItemTable")
if not g then
	h = {}
end
local i = h and h.items or h or {}
local function j(k)
	local l = {}
	for m, n in ipairs(i) do
		if n.tier == k then
			table.insert(l, n.name)
		end
	end
	return l
end
...
local t = {
	...
	{ name = "item_enhancement_feverish", tier = 5, roles = { 1, 1, 1, 1, 1 }, realName = "Feverish Enhancement" },
	{
		...
		tier = 5,
		roles = { 1, 1, 1, 1, 1 },
		realName = "Fleetfooted Enhancement",
	},
...
function NeutralItems:GetRandomEnhanByTier(k)
	local u = {}
	for m, v in ipairs(t) do
		if v.tier == k then
			table.insert(u, v)
		end
	end
...
function NeutralItems.GiveNeutralItems(w, x)
	local y = Helper.IsTurboMode()
	if (y and Helper.DotaTime() >= 0 or Helper.DotaTime() >= 0) and not a then
		GameRules:SendCustomMessage("Bots receiving Tier 1 Neutral Items...", 0, 0)
		for m, z in pairs(w) do
			NeutralItems.GiveItem(o[RandomInt(1, #o)], z, a, 1)
		end
		for m, z in pairs(x) do
			NeutralItems.GiveItem(o[RandomInt(1, #o)], z, a, 1)
		end
		a = true
	end
	if (y and Helper.DotaTime() >= 8.5 * 60 or Helper.DotaTime() >= 17 * 60) and not b then
		GameRules:SendCustomMessage("Bots receiving Tier 2 Neutral Items...", 0, 0)
		for m, z in pairs(w) do
			NeutralItems.GiveItem(p[RandomInt(1, #p)], z, a, 2)
		end
		for m, z in pairs(x) do
			NeutralItems.GiveItem(p[RandomInt(1, #p)], z, a, 2)
		end
	...
	if (y and Helper.DotaTime() >= 13.5 * 60 or Helper.DotaTime() >= 27 * 60) and not c then
		GameRules:SendCustomMessage("Bots receiving Tier 3 Neutral Items...", 0, 0)
		for m, z in pairs(w) do
			NeutralItems.GiveItem(q[RandomInt(1, #q)], z, b, 3)
		end
		for m, z in pairs(x) do
			NeutralItems.GiveItem(q[RandomInt(1, #q)], z, b, 3)
		end
	...
	if (y and Helper.DotaTime() >= 18.5 * 60 or Helper.DotaTime() >= 37 * 60) and not d then
		GameRules:SendCustomMessage("Bots receiving Tier 4 Neutral Items...", 0, 0)
		for m, z in pairs(w) do
			NeutralItems.GiveItem(r[RandomInt(1, #r)], z, c, 4)
		end
		for m, z in pairs(x) do
			NeutralItems.GiveItem(r[RandomInt(1, #r)], z, c, 4)
		end
	...
	if (y and Helper.DotaTime() >= 30 * 60 or Helper.DotaTime() >= 60 * 60) and not e then
		GameRules:SendCustomMessage("Bots receiving Tier 5 Neutral Items...", 0, 0)
		for m, z in pairs(w) do
			NeutralItems.GiveItem(s[RandomInt(1, #s)], z, d, 5)
		end
		for m, z in pairs(x) do
			NeutralItems.GiveItem(s[RandomInt(1, #s)], z, d, 5)
		end
...
function NeutralItems.GiveItem(A, B, C, D)
	if A == nil then
		log("[Buff][NeutralItems] tier %s: nil itemName (empty tier list?)", tostring(D))
		return
	end
	NeutralItems:RemoveEnhan(B)
	if B:HasRoomForItem(A, true, true) then
		local n = CreateItem(A, B, B)
		if n == nil then
			log(
				'[Buff][NeutralItems] tier %s: CreateItem("%s") returned nil -- invalid/renamed in this patch',
				tostring(D),
				tostring(A)
			)
			return
		...
		n:SetPurchaseTime(0)
		if NeutralItems.HasNeutralItem(B) and C then
			B:RemoveItem(B:GetItemInSlot(f))
			NeutralItems:RemoveEnhan(B)
			B:AddItem(n)
		else
			B:AddItem(n)
		end
		local E = NeutralItems:GetRandomEnhanByTier(D)
		if E then
			local F = CreateItem(E.name, B, B)
			if F == nil then
				log(
					'[Buff][NeutralItems] tier %s: CreateItem enhancement "%s" returned nil',
					tostring(D),
					tostring(E.name)
				)
			else
...
function NeutralItems:RemoveEnhan(G)
	for H = 1, 20 do
		local I = G:GetItemInSlot(H)
		if I ~= nil then
			if string.find(I:GetName(), "item_enhancement") then
				G:RemoveItem(I)
			end
		end
	end
end
function NeutralItems.HasNeutralItem(B)
	if not B then
		return false
	end
	local n = B:GetItemInSlot(f)
	if n then
		return true
	end
	return false
